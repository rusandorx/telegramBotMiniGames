...
async def wordle_exit(update, context):
    context.user_data.clear()
    await update.message.reply_text('Спасибо за игру', reply_markup=ReplyKeyboardRemove())
...
async def wordle_difficulty(update, context):
    ...
    if not difficulty:
        ...
        return 1
    await update.message.reply_text(f'Выбрана сложность {update.message.text}', reply_markup=ReplyKeyboardRemove())
...
async def wordle_answer(update, context):
    ...
    if ans == context.user_data['wordle_word']:
        await update.message.reply_text(f'Верно\n'
                                        ...
                                        f'Попыток - {len(context.user_data["wordle_tries"]) - 1}\\{context.user_data["difficulty"][2]}\n' +
    ...
    if len(context.user_data['wordle_tries']) > context.user_data['difficulty'][2]:
        await update.message.reply_text('Вы не смогли угадать слово.\n'
                                        f'Слово: {context.user_data["wordle_word"]}\n'
                                        f'Попробовать еще раз? - /wordle', reply_markup=ReplyKeyboardRemove())
        context.user_data.clear()
        return ConversationHandler.END
